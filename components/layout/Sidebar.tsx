...
export default function Sidebar({ active, onSelect }: Props) {
  return (
    <aside className="sidebar-bg fixed left-0 top-0 bottom-0 z-30 w-16 flex flex-col items-center py-4 gap-2 border-r border-neural-border/60">
      {/* Logo mark */}
      <div className="w-8 h-8 border border-neural-primary/70 flex items-center justify-center mb-4 shadow-[0_0_12px_rgba(0,212,255,0.2)]">
          ...
          return (
            <div key={tool.id} className="relative group/item">
              ...
              >
                ...
                {isActive && (
                  <div className="absolute left-0 top-2 bottom-2 w-[3px] bg-neural-primary shadow-[0_0_8px_rgba(0,212,255,0.8)]" />
                )}
                {/* Hover left bar */}
                {!isActive && (
                  <div className="absolute left-0 top-3 bottom-3 w-[2px] bg-neural-primary/0 group-hover/item:bg-neural-primary/30 transition-all duration-150" />
                )}
                <span className={`text-base leading-none transition-all ${isActive ? 'drop-shadow-[0_0_6px_rgba(0,212,255,0.8)]' : ''}`}>
                  {tool.symbol}
                </span>
                <span className={`text-[8px] font-display tracking-widest ${isActive ? 'text-neural-primary' : ''}`}>
                  {tool.short}
                </span>
              </button>
